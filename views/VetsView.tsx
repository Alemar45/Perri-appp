...
const VetsView: React.FC = () => {
  ...
  const performSearch = useCallback(async (term: string, lat?: number, lng?: number) => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        setClinics(results);
        setCurrentLocation(lat && lng ? 'Mi Ubicación' : term);
      }
    } catch (err) {
  ...
  const handleGeoLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => performSearch("clínicas cercanas", pos.coords.latitude, pos.coords.longitude),
      () => setIsLoading(false)
    );
  };

  useEffect(() => {
    if (clinics.length <= 2) performSearch('Buenos Aires');
  }, [performSearch]);
    ...
    <div className="pb-32 min-h-screen bg-background-light dark:bg-background-dark">
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md p-6 border-b border-gray-100 dark:border-zinc-800 shadow-sm">
        ...
        <div className="flex gap-3">
          <button onClick={handleGeoLocation} className="size-14 bg-white dark:bg-zinc-800 border-2 border-primary/20 rounded-[1.25rem] flex items-center justify-center text-primary shadow-sm">
          ...
          <div className="relative flex-1">
            <input 
              ...
              className="w-full h-14 bg-gray-50 dark:bg-zinc-800 border-none rounded-[1.25rem] px-6 text-sm font-bold shadow-inner"
              placeholder="Buscar ciudad..."
            />
          </div>
          <button 
            onClick={() => setFilter24h(!filter24h)}
            className={`size-14 rounded-[1.25rem] flex items-center justify-center border-2 ${filter24h ? 'bg-red-500 border-red-500 text-white shadow-lg' : 'bg-white dark:bg-zinc-800 border-gray-100 dark:border-zinc-700 text-gray-400'}`}
          >
      ...
      <div className="p-6 space-y-4">
        {isLoading ? (
          <div className="text-center py-20 text-gray-400 font-black text-[10px] uppercase tracking-widest">Cargando veterinarias...</div>
          ...
          <div key={vet.id} className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] shadow-sm border border-gray-50 dark:border-zinc-800">
            <div className="flex justify-between items-start mb-2">
              ...
              <span className={`text-[8px] font-black px-2 py-1 rounded-full uppercase ${vet.is24h ? 'bg-red-500 text-white' : 'bg-emerald-500 text-white'}`}>
                {vet.is24h ? '24H' : 'Normal'}
              </span>
            </div>
            <p className="text-xs text-gray-500 mb-4">{vet.address}</p>
            <div className="flex gap-2">
              <a href={`tel:${vet.phone}`} className="flex-1 h-12 bg-primary text-black rounded-xl flex items-center justify-center font-black text-[10px] uppercase shadow-md">
                Llamar ahora
              </a>
            </div>
          </div>
